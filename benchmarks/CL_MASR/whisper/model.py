...
from adapters.src.adapters import setup_adapter_training
from adapters.src.adapters.models.whisper import WhisperAdapterModel
...
class ProgressiveWhisperWithAdapters(WhisperAdapterModel):
    """Whisper model that supports decoding with predicted batch of languages,
    implements a more efficient decoding and whose tokenizer's vocabulary can
    be progressively extended by adding new tokens.
    ...
    # override
    def __init__(
        self, source, save_path, adapter_config, **kwargs,
    ):
        super().__init__(
            source, save_path, **kwargs,
        )
        ...
        #################################################################
        # Create adapters
        setup_adapter_training(self, adapter_config, "CL-MASR")

        # Remove adapters in encoder
        for layer in self.model.encoder.layers:
            layer.attention_adapters.adapters = nn.ModuleDict()
            layer.output_adapters.adapters = nn.ModuleDict()
